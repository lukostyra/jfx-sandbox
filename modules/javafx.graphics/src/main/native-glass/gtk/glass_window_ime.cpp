...
bool WindowContext::im_filter_keypress(XKeyEvent* xevent) {
    ...
    int len = Xutf8LookupString(xim.ic, (XKeyPressedEvent *) &xevent, buffer, buf_len - 1, &keysym, &status);
    if (status == XBufferOverflow) {
        buf_len = len + 1;
        buffer = (char*)realloc(buffer, buf_len * sizeof (char));
        len = Xutf8LookupString(xim.ic, (XKeyPressedEvent *) &xevent, buffer, buf_len - 1,
                &keysym, &status);
    }
    switch (status) {
        case XLookupKeySym:
...
bool WindowContext::filterIME(XEvent* xevent) {
    ...
    switch (xevent->type) {
        case KeyPress:
        case KeyRelease:
            return im_filter_keypress(&xevent->xkey);
        default:
            return FALSE;
    }
}
