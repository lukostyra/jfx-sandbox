...
//    jint mh = monitor_geometry.height / uiScale;
    jint mx = monitor_geometry.x;
    jint my = monitor_geometry.y;
    jint mw = monitor_geometry.width;
    jint mh = monitor_geometry.height;
...
//
    ...
    g_print("Monitor: x %d, y %d, w %d, h %d\n", mx, my, mw, mh);
...
//    try {
//        return createJavaScreen(env, default_display, monitor_idx);
//    } catch (jni_exception&) {
//        return NULL;
//    }
//}
...
void display_settings_changed(GdkDisplay* disp, gchar* setting, gpointer user_data) {
    (void)disp;
    (void)user_data;

    //TODO: maybe check which setting changed
