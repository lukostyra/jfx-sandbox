...
//FIXME: Gtk4 there are many more keys
...
jint get_glass_key(GdkEvent* e) {
    init_keymap();

//    guint keyValue;
//    guint state = gdk_event_get_modifier_state(GDK_EVENT(e)) & GDK_LOCK_MASK; //NumLock test
    guint keycode = gdk_key_event_get_keycode(e);

//    gdk_keymap_translate_keyboard_state(gdk_keymap_get_for_display(gdk_display_get_default()),
//            keycode, static_cast<GdkModifierType>(state), NULL, //FIXME e->group,
//            &keyValue, NULL, NULL, NULL);


    jint key = GPOINTER_TO_INT(g_hash_table_lookup(keymap, GINT_TO_POINTER(keycode)));
...
jint gdk_modifier_mask_to_glass(guint mask)
{
    ...
    glass_mask |= (mask & GDK_ALT_MASK) ? com_sun_glass_events_KeyEvent_MODIFIER_ALT : 0;
